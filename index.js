...
🎮 الألعاب`
  ...
  cmd.startsWith("اغنيه ") ||
  cmd.startsWith("حنكش عايز اغنيه ")
) {
  
let query = "";

if (cmd.startsWith("اغنيه ")) {
  query = text.slice(6).trim();
} else {
  query = text.slice("حنكش عايز اغنيه ".length).trim();
}
  if (!query) {
    await sock.sendMessage(jid, {
      text: "اكتب اسم الأغنية بعد كلمة اغنيه."
    });
    return;
  }
...
} else if (text.toLowerCase().startsWith("حنكش ")) {

    const question = text.slice(5).trim();

    if (!question) {
        await sock.sendMessage(jid, {
            text: "اكتب سؤالك بعد كلمة حنكش."
        });
        return;
    }
